...
const API_KEY =
  'ZMRzv1q2%2BYzORg2CS1IZhYglrlF1WoYDe6dzK9UPGDA59kO5GgN2V9NEniLd0bvLIVmgW6WOiooTcUSLX%2FJ16Q%3D%3D';
...
const base_date = `${timeChecker()[0]}${today(timeChecker()[1])}${today(
  timeChecker()[2]
...
function Weather() {
  ...
  const getData = async (x, y) => {
    ...
    const {
      ...
      `${WeatherEndpoint}?serviceKey=${API_KEY}&dataType=JSON&numOfRows=10&pageNo=1&base_date=${base_date}&base_time=${base_time}&nx=${x}&ny=${y}`
    );
